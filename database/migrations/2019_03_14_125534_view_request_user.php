...
class ViewRequestUser extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        DB::statement("
            ...
            SELECT
                ...
                `response_for_users`.`status` AS `response_for_users_status`,
            ...
            FROM
                `request_users`
            LEFT JOIN `response_for_users` ON `request_users`.`id` = `response_for_users`.`request_id`
            UNION ALL
            SELECT
                `request_users`.`id` AS `request_users_id`,
                `request_users`.`user_id` AS `request_users_user_id`,
            ...
            FROM
                `request_users`
            RIGHT JOIN `response_for_users` ON `request_users`.`id` = `response_for_users`.`request_id`
            WHERE
                `request_users`.id IS NULL
        ");
    }
    ...
    {
        Schema::dropIfExists('view_request_user');
    }
}
